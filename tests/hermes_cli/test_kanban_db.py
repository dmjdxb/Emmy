...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Emmy profile — preserves the
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Emmy' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Emmy' DB/metadata/logs on task completion.
    """
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "Emmy Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
