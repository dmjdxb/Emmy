...
version: 0.1.0
author: Matt Maximo (MattMaximo), Emmy
license: MIT
...
# Grok Build CLI — Emmy Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Emmy terminal. Grok
...
1. Prepare stable input files first with Emmy tools (`read_file`,
...
2. **Don't conflate Emmy' xAI auth with the `grok` CLI's auth.** Emmy'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Emmy' xAI auth carries
   over.
