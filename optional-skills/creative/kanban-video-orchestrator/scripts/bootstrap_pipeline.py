...
../assets/, and writes a setup.sh that creates Emmy profiles and fires the
initial kanban task.
    ...
    if "team" in plan:
        if not isinstance(plan["team"], list) or not plan["team"]:
            ...
            for i, t in enumerate(plan["team"]):
                ...
                # Profile name must match Emmy's regex (lowercase
                ...
                if "profile" in t:
                    if not PROFILE_NAME_RE.match(t["profile"]):
                        errors.append(
                            ...
                            f"[a-z0-9][a-z0-9_-]{{0,63}} per Emmy profile rules"
                        )
