...
``<root>`` is the **shared Emmy root** (the parent of any active
...
the profile's Emmy home but are otherwise isolated: a worker spawned
...
def kanban_home() -> Path:
    """Return the shared Emmy root that anchors the kanban board.
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Emmy treats as user-controlled-but-trusted on the
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Emmy' own DB, metadata, and logs, not task workspaces.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Emmy
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Emmy profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Emmy profile.
        # `_default_spawn` invokes ``hermes -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Emmy
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Emmy CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Emmy shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Emmy executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m robin.main`` — fallback for setups where
       Emmy is launched from a venv and the ``hermes`` shim is not on
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Emmy or activate its venv before running the kanban dispatcher."
        )
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Emmy root exists
