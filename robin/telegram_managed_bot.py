...
manual BotFather token copy-paste. Emmy talks only to the EnergyIR onboarding
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Emmy Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Emmy Telegram onboarding service.")
