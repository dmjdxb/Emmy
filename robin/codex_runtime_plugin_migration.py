"""Migrate Emmy' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Emmy' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Emmy mcp_servers.<n>.command/args/env  → codex stdio transport
  Emmy mcp_servers.<n>.url/headers       → codex streamable_http transport
  Emmy mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Emmy mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec
...
What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Emmy' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Emmy config.")
...
# Emmy keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Emmy' sampling subsection — codex MCP has no equivalent
    "sampling",
}
...
def _translate_one_server(
    ...
    """Translate one Emmy MCP server config to the codex inline-table dict
    ...
    if has_command:
        ...
        # Emmy' transport: sse hint is informational; codex auto-negotiates
    ...
    for key in hermes_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            ...
            skipped.append(f"{key} (unknown Emmy key)")
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Emmy)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Emmy' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Emmy' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Emmy
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _build_hermes_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Emmy' own
...
def migrate(
    ...
    *,
    ...
    """Translate Emmy mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.
    ...
    Args:
        ...
        expose_hermes_tools: when True (default), register Emmy' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Emmy for tools
    ...
    if not isinstance(hermes_servers, dict):
        report.errors.append(
            "mcp_servers in Emmy config is not a dict; cannot migrate."
        )
        return report
    ...
    # Inject Emmy' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Emmy for the tools codex
