 ...
 *
 * Stamps the Emmy icon + identity onto the packed Windows Emmy.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Emmy')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Emmy'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Emmy.exe keeps the stock Electron icon`)
  }
}
