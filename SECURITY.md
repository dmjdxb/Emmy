# Emmy Security Policy

This document describes Emmy's trust model, names the one
...
security vulnerabilities. **Emmy does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Emmy access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Emmy's documentation
...
Emmy supports two OS-level isolation postures. They address
...
Emmy supports this in two ways:

- **Emmy's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Emmy's in-process heuristics
...
Emmy filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Emmy
itself. Bugs in Emmy's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Emmy does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Emmy's own documentation, or reasonable
  operator expectations would predict — including cases where
  Emmy has documented a stance about how its output should
...
- Emmy includes supply-chain guards for MCP server
