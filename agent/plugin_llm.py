...
Plugins built on Emmy often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Emmy
...
embedded slugs, no shorthands. This mirrors Emmy' main config
...
handles every provider, fallback chain, and per-task override Emmy
supports.
"""
