...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Emmy tool use: "
    ...
    return (
        ...
        f"context, but Emmy needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Emmy config, set `model.ollama_num_ctx: 65536` "
...
def run_conversation(
    ...
    # are reused.  The system prompt is Emmy's territory; plugins
    ...
    # all run inside Codex). Default Emmy path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Emmy internals.
        #
        # Emmy invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Emmy tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                # providers (DeepSeek, Kimi, MiMo, Emmy).  A 429 can
                ...
                # which Emmy' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Emmy' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
