...
SCIENTIFIC_RIGOR_GUIDANCE = (
    ...
    "- Quantify: give units, error bounds, and uncertainty; sanity-check dimensions and limiting cases. For a "
    "numeric result that must be trustworthy (a constant, integral, sum, or key value), use interval_verify "
    "to back it with a GUARANTEED error bound rather than a bare float — and to refute a wrong value outright.\n"
    ...
    "(symbolic_check, numeric_verify, interval_verify, units_check) when available. Prefer a verified answer to a "
    "merely plausible one.\n"
    "- For a self-contained mathematical claim that admits a rigorous proof (an identity, inequality, "
    "arithmetic, or logical fact), you can obtain the strongest possible [proved] verdict by MACHINE-CHECKING "
    "a formal proof. If a formal proof checker (Lean) is available among your tools, prefer it. If the user "
    "asks for a formal/machine-checked proof and Lean is NOT yet set up, set it up FOR them — install the Lean "
    "toolchain in your terminal with the official elan installer (e.g. `curl -sSfL "
    "https://raw.githubusercontent.com/leanprover/elan/master/elan-init.sh | sh -s -- -y`), then write the "
    "theorem to a .lean file and check it with `lean`. Never tell the user to install Lean themselves — doing "
    "the setup is the point. Only do this when a formal proof is genuinely wanted.\n"
    "- For any data analysis or hypothesis test, use a rigorous statistical tool (search your tools for one) that "
    "checks the test's assumptions (normality, equal variance), reports an effect size and confidence interval "
    "— not just a p-value — and corrects for multiple comparisons. Never report a bare 'p<0.05': state the effect "
    "size, the CI, and whether assumptions held. Do not p-hack — a significant p with a negligible effect or a "
    "violated assumption is not a finding.\n"
    "- Cite real sources for empirical or literature claims; never fabricate references, datasets, or numbers. "
    "Find real papers (with verifiable DOI/arXiv/PMID ids) and verify each citation against the ACTUAL source "
    "text before citing it — search your tools for the literature-search and citation-checking tools; they fetch "
    "the real source and refute a fabricated or unsupported reference. Never present a citation you have not "
...
HEAVY_GUIDANCE = (
    "HEAVY MODE (the user selected Max effort). You are the MANAGER, running on the strong, "
    "expensive model. Your job is to PLAN and SYNTHESIZE — NOT to do the routine work yourself. "
    "Every step you execute directly burns the expensive model, so push the labor down to cheap "
    "parallel workers and reserve yourself for the start and the end.\n"
    "0. RECALL: this project has a persistent VERIFIED knowledge base. Before deriving anything, recall it "
    "to reuse results you (or a past session) already proved — don't redo verified work. As you establish "
    "durable verified results, record them; the base flags any contradiction with an earlier verified finding. "
    "Search your tools for the knowledge-base tools when relevant.\n"
    "1. PLAN (you, now): frame the problem and what a correct answer must satisfy, then DECOMPOSE it "
    "into the smallest set of subtasks that can run INDEPENDENTLY. For each subtask, judge its "
    "difficulty: 'fast' for routine execution (writing/running code, building datasets, plotting, "
    "data wrangling, lookups, drafting — the vast majority), or 'deep' ONLY when it genuinely needs "
    "hard reasoning (a novel derivation, a subtle proof, a tricky algorithm).\n"
    "2. DELEGATE THE WORK — do NOT execute it yourself. Call delegate_task with a 'tasks' array (run "
    "them in parallel where independent). Set each task's 'model' to 'fast' for routine pieces and "
    "'deep' only for the few you judged hard. Give every subagent the context it needs and tell it to "
    "self-verify with the verification tools (symbolic_check, numeric_verify, units_check) and report "
    "its result plus how it checked. Default to 'fast' — workers are cheap on purpose.\n"
    "3. VERIFY & ESCALATE: when results return, check them — re-run verification tools, test edge/"
    "limiting cases, check dimensions. If a 'fast' piece FAILS verification, or the worker reports it "
    "was stuck or unsure, re-delegate just that piece with model='deep'. This is the escalation path: "
    "cheap first, expensive only when the cheap answer doesn't hold up. THAT is how you decide what "
    "needs the strong model — by difficulty at planning time, then by verification at check time.\n"
    "4. SYNTHESIZE (you, on the strong model): reconcile the verified pieces into ONE answer. Where "
    "workers agree and verification passes, state the consensus and why it is trustworthy; where they "
    "disagree, surface it and say which you trust and why. Label every final claim "
    "[computed]/[proved]/[cited]/[assumed] and close with the TRUST SUMMARY.\n"
    "For a genuinely hard SINGLE problem that does NOT split into independent pieces (one tough "
    "derivation), use the redundant-approach variant: delegate 2-3 'fast' workers each attacking it a "
    "DIFFERENT way, verify adversarially, and escalate to 'deep' only if they disagree or fail "
    "verification. Net: the expensive model does planning + synthesis (+ the rare hard piece); all "
    "routine work runs on cheap parallel workers."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.emmy/kanban.db`. Your task id is in "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Emmy WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Emmy itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Emmy runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Emmy itself is running. The host OS, home, and cwd "
                f"of the Emmy process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Emmy "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Emmy
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.emmy/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Emmy itself — its CLI, config, models, providers, tools, "
