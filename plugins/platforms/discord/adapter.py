...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Emmy manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Emmy session")
        ...
        @tree.command(name="status", description="Show Emmy session status")
        ...
        @tree.command(name="stop", description="Stop the running Emmy agent")
        ...
        @tree.command(name="update", description="Update Emmy to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Emmy gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Emmy session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Emmy in the thread",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Emmy session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Emmy: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Emmy"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Emmy: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Emmy handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
