"""langfuse — Emmy plugin for Langfuse observability.

Traces Emmy conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Emmy plugin system — standalone plugins only
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Emmy plugin system —
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Emmy branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Emmy also has a turn-scoped
    ...
    # messages is a list only for legacy Emmy branches that fired
    # pre_llm_call with API messages directly. Current Emmy fires
...
def register(ctx) -> None:
    ...
    # Emmy versions.  pre_api_request / post_api_request fire per API
