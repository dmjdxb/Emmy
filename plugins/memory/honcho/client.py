...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Emmy instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Emmy profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Emmy profile.
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Emmy system prompt
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Emmy profile.
        """
        resolved_host = host or resolve_active_host()
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Emmy session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Emmy session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Emmy session_id (new Honcho session each run)
